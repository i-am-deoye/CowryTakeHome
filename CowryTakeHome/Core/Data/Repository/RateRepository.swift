...
fileprivate extension DefaultRateRepository {
    ...
    func onFetchedSymbols(_ result: HTTPClient.Result) {
        self.onResponse(result) {[weak self] data in
            guard let self = self else { return }
            guard let response = data.decode(key: "symbols") else {
                self.symbolsResultCompletion?(.failure(.general))
                return
            }
            
            let symbols = RSymbols.init(json: response.data)
    ...
    func onLatest(_ result: HTTPClient.Result) {
        ...
        self.onResponse(result) {[weak self] data in
            guard let self = self else { return }
            guard let response = data.decode(key: "rates", keys: [baseKey]) else {
                self.rateResultCompletion?(.failure(.general))
                return
            }
